...
if (!channelId) {
    console.error("Missing result.channel_id");
    process.exit(1);
}
...
function maybePatchWitnessWithSignature(transaction, signatures) {
    ...
    const signatureHex = pickSignatureHex(signatures);
    if (!signatureHex) {
        return;
    }
...
try {
    ...
    maybePatchWitnessWithSignature(signedFundingTx, signedTxFile?.signatures);
